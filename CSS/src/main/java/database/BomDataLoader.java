...
public class BomDataLoader {
	...
	public static void LoadBomData(Session session)
	{
		...
		file = new File(bomPictureFolderPath + "trend-mean-temperature-1970-2011.png");
		...
		file = new File(bomPictureFolderPath + "trend-maximum-temperature-1970-2011.png");
		try { arrPictureContent = FileUtils.readFileToByteArray(file); }
		catch (IOException e) { arrPictureContent = null; e.printStackTrace(); }
		...
		file = new File(bomPictureFolderPath + "trend-total-annual-rainfall-1970-2011.png");
		try { arrPictureContent = FileUtils.readFileToByteArray(file); }
		catch (IOException e) { arrPictureContent = null; e.printStackTrace(); }
		...
		file = new File(bomPictureFolderPath + "short-term-sea-level-changes.png");
		try { arrPictureContent = FileUtils.readFileToByteArray(file); }
		catch (IOException e) { arrPictureContent = null; e.printStackTrace(); }
		session.save(new PastData("Short-term changes in sea level", datePastData, dateShortTermPastStart, dateShortTermPastEnd, "http://www.csiro.au/Outcomes/Climate/Understanding/State-of-the-Climate-2012.aspx", arrPictureContent));
